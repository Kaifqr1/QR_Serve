...
| `VITE_OAUTH_PORTAL_URL` | Yes | OAuth portal used by the browser login bootstrap. |
| `OWNER_OPEN_ID` | Recommended | Existing project owner identifier for the bootstrap admin role. |
...
| OAuth | Callback uses the current browser origin | Register `https://YOUR_DOMAIN/api/oauth/callback` with the OAuth provider for every production domain. Add preview callbacks only if the provider supports them safely. |
...
2. Add the required environment variables and register the production OAuth callback URL.
3. Provision external database and image-storage providers, migrate the two managed integrations, and run database migrations from a controlled CI/administrative job.
