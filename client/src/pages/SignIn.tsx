...
export default function SignIn() {
  ...
  const signIn = trpc.auth.signIn.useMutation({
    ...
    onError: error => toast.error(error.data?.code === "UNAUTHORIZED" ? "The email address or password is not valid." : error.message),
  });
        ...
        <div className="w-full max-w-md">
          ...
          <div className="rounded-[2rem] border border-white/10 bg-[#211f1c] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.25)] sm:p-10">
            ...
            <form onSubmit={submit} className="mt-8 space-y-5">
              ...
              <Button type="submit" disabled={signIn.isPending} className="mt-2 h-12 w-full rounded-full bg-[#ed5739] text-white hover:bg-[#d94830]">{signIn.isPending ? "Please wait…" : "Sign in"}<ArrowRight className="ml-2 h-4 w-4" /></Button>
            </form>
          </div>
        </div>
      </main>
